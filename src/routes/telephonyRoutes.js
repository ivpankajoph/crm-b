...
import { validatePlivoWebhook } from '../middleware/plivoWebhookMiddleware.js';
import { answerBrowserCall, answerClickToCall, answerIncomingCallback, buyNumber, configureInboundCallbacks, finalizeBrowserCall, getBrowserSession, getCallLogDetails, getCurrentIncomingCall, getTelephonyConfig, hangupClickToCall, incomingCallHangup, listCallLogs, recordingCallback, retryCallTranscription, searchNumbers, selectNumber, transcriptionCallback, updateActiveCallingPool } from '../controllers/telephonyController.js';
...
router.post('/webhooks/incoming', validatePlivoWebhook, answerIncomingCallback);
router.post('/webhooks/incoming-hangup', validatePlivoWebhook, incomingCallHangup);
...
router.get('/browser/session', protect, requirePermission(PERMISSIONS.CALLS_MANAGE), getBrowserSession);
router.get('/browser/incoming/current', protect, requirePermission(PERMISSIONS.CALLS_MANAGE), getCurrentIncomingCall);
...
router.post('/numbers/configure-callbacks', protect, adminOnly, configureInboundCallbacks);
