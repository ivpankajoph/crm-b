import { Contact, InterestClassificationLog } from "../../storage/mongodb.adapter.js";
import { DripCampaign, DripRun } from "../drips/drip.model.js";
import { autoEnrollContact } from "../drips/drip.service.js";
...
class InterestClassificationService {
  genAI = null;
  constructor() {
  ...
  async classifyMessage(messageContent, contactPhone, userId, useAI = true) {
    const lowerMessage = messageContent.toLowerCase().trim();
    const foundInterestedKeywords = INTERESTED_KEYWORDS.filter(
      (kw) => lowerMessage.includes(kw.toLowerCase())
    );
    const foundNotInterestedKeywords = NOT_INTERESTED_KEYWORDS.filter(
      (kw) => lowerMessage.includes(kw.toLowerCase())
    );
...
{"status": "interested|not_interested|neutral", "confidence": 0.0-1.0, "reason": "brief explanation"}`;
  ...
  async classifyAndUpdateContact(messageContent, contactId, contactPhone, userId) {
...
const interestClassificationService = new InterestClassificationService();
export {
  InterestClassificationService,
  interestClassificationService
};
