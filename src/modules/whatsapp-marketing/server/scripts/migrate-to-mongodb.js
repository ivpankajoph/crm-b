...
async function migrate() {
  ...
  try {
    ...
    if (data.contacts?.length > 0) {
      ...
      const existingContacts = await mongodb.findMany("contacts", {});
    ...
    if (data.messages?.length > 0) {
      ...
      const existingMessages = await mongodb.findMany("messages", {});
    ...
    if (data.campaigns?.length > 0) {
      ...
      const existingCampaigns = await mongodb.findMany("campaigns", {});
    ...
    if (data.templates?.length > 0) {
      ...
      const existingTemplates = await mongodb.findMany("templates", {});
    ...
    if (data.automations?.length > 0) {
      ...
      const existingAutomations = await mongodb.findMany("automations", {});
      const existingIds = new Set(existingAutomations.map((a) => a.id));
    ...
    if (data.teamMembers?.length > 0) {
      ...
      const existingMembers = await mongodb.findMany("team_members", {});
    ...
    const contacts = await mongodb.findMany("contacts", {});
    const messages = await mongodb.findMany("messages", {});
    const existingChats = await mongodb.findMany("chats", {});
    const existingChatContactIds = new Set(existingChats.map((c) => c.contactId));
    const newChats = [];
